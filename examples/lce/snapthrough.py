...
class LCESnapThroughAssembly:
    def __init__(self, T0: float = 20, kr: float = 1.0):
        T = Symbol('T')
        k = 0.12 * (2.3 - 2.25 / (1 + exp(-0.31 * (T - 47.06))) + 0.0005 * (T - 40) ** 2)
        l0 = (0.1 + 20 / (1 + exp(-0.208 * (T - 54.5)))) / 10

        self.lce = SpringT(l0, k)
        # self.spring = Spring(k=kr*self.lce.k(T0), l0=self.lce.l0(T0))
        self.spring = InclinedTruss()
...
controller = Adaptive(value=0.1, min=0.001, max=1, decr=0.1, incr=1.5)
